...
#from datafold.dynfold import (GeometricHarmonicsInterpolator as GHI, LaplacianPyramidsInterpolator as LPI, TSCRadialBasis, LocalRegressionSelection, TSCTakensEmbedding)
...
def Embed(method, X_train_local, target_intrinsic_dim, **kwargs):
    ...
    if "DM" in method:
        ...
        out = [target_mapping, evecs, explainedVarianceRatio, evals]
    ...
    for i in tqdm(range(trainSetLength, data.shape[0], 1)):
        ...
        try:
            ...
            if liftMethod == 'FullModel':
                ...
                if "PCA" in embedMethod:
                    single_lifted_Preds_pca = np.dot(row_Preds, roll_parsimoniousEigs.T)
                    single_lifted_Preds = single_lifted_Preds_pca[0]
                else:
                    single_lifted_Preds = Lift("GH", roll_X_train_lift, roll_X_test, roll_target_mapping, row_Preds,
...
if __name__ == '__main__':
    ...
    embedMethod = "PCA"  # LLE, DMComputeParsimonious, DM, PCA

    #pcFolderRoot = 'D:\Dropbox\VM_Backup\\'
    #pcFolderRoot = 'E:\Dropbox\Dropbox\VM_Backup\\'
    pcFolderRoot = 'F:\Dealing\Panagiotis Papaioannou\AcademicsCode\Epidemics_Paper2\\'
    ...
    if label == "EEGsynthetic2":
        ...
        datasetsPath = pcFolderRoot + 'CodePack0\DataSets_FOREX_officePC\\'
        RollingRunnersPath = pcFolderRoot + 'CodePack0\RollingRunners_FOREX_officePC\\'
