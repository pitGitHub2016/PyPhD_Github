...
def gDMAP_TES(mode, universe, alphaChoice, lifting):
    ...
    if mode == 'create':
        ...
        if weightSpace[0] == 1:
            ...
            #dmapsCompRatesDF = sl.sign(sl.rs(dmapsCompRatesDF))
            dmapsCompRatesDF = sl.cs(sl.rs(dmapsCompRatesDF))

        if weightSpace[1] == 1:
            ###################### TEMPORAL WEIGHTS #######################
            sigDriver = sl.fd(pd.read_sql('SELECT * FROM gDMAP_TES_AssetsRets_' + str(alphaChoice) + "_LinearRegression_0", conn).set_index('Dates', drop=True)).fillna(0)
            #sigDriver = sl.rs(sl.fd(pd.read_sql('SELECT * FROM gDMAP_TES_Rates_' + str(alphaChoice) + "_0", conn).set_index('Dates', drop=True))).fillna(0)

            for pr in range(1,5):
                sigDriver += sl.S(sl.fd(pd.read_sql('SELECT * FROM gDMAP_TES_AssetsRets_' + str(alphaChoice) + "_LinearRegression_" + str(pr), conn).set_index('Dates', drop=True))).fillna(0)
            ...
            #sigDriver = sl.cs(sigDriver)
            # sigDriver = sl.sign(sigDriver)

        if weightSpace[2] == 1:
            sigDriverTemporal = pd.read_sql('SELECT * FROM gDMAP_TES_AssetsRets_' + str(alphaChoice) + "_Temporal", conn).set_index('Dates', drop=True)
            sigDriverTemporal = sigDriverTemporal.iloc[:,0]
            #sigDriverTemporal = sl.rs(sigDriverTemporal)
            #sigDriverTemporal = sl.cs(sigDriverTemporal)
            #sigDriverTemporal = sl.sign(sigDriverTemporal)

        ####################################################################

        #sig = dmapsCompAssetRetsDF
        #sig = dmapsCompRatesDF
        sig = sigDriver.copy()
        #sig = dmapsCompAssetRetsDF * (-1) + sigDriver * (-1)
        #sig = dmapsCompRatesDF + sl.rs(sigDriver) # sign(rs) + cs --> gold
        #sig = sigDriverTemporal.copy()

        sig = sl.S(sig)

        predictDFAssets = df.mul(sig, axis=0).fillna(0)
        predictDF = predictDFAssets
        #predictDF = sl.Strategies.EMA_signal(predictDFAssets, nIn=25)
...
#gDMAP_TES("trade", "AssetsRets", 'sumKLMedian', 'Temporal')
