...
def DataHandler(mode):
    ...
    if mode == 'run':
        ...
        fig, ax = plt.subplots()
        (sl.ecs(rets)*100).plot(ax=ax)
        box = ax.get_position()
        ax.set_position([box.x0, box.y0, box.width * 0.9, box.height])
        mpl.pyplot.ylabel("Assets Cumulative Returns (%)")
        plt.legend(bbox_to_anchor=(1.01, 1), loc=2, frameon=False, prop={'size': 12}, borderaxespad=0.)
        plt.show()

        fig, ax = plt.subplots()
        (Prices[BondsTickers]*100).plot(ax=ax)
        box = ax.get_position()
        ax.set_position([box.x0, box.y0, box.width * 0.9, box.height])
        mpl.pyplot.ylabel("Rates (%)")
...
def RiskParity():
    ...
    SRollVol = np.sqrt(252) * sl.S(sl.rollStatistics(df, method='Vol', nIn=250)) * 100
    ...
    df.loc["2006-01-06 00:00:00", "S&P 500"] = 0
...
def RunRollManifold(universe):
    ...
    for tw in twList:
        ...
        out[3].to_sql(manifoldIn + "_" + universe + '_lambdasDf_tw_' + str(tw), conn, if_exists='replace')
...
def ProjectionsPlots(manifoldIn, universe):
    ...
    for tw in twList:
        ...
        #exPostProjections.columns = ['$\Pi_{'+manifoldIn+','+str(tw)+',1,t}$','$\Pi_{'+manifoldIn+','+str(tw)+',2,t}$', '$\Pi_{'+manifoldIn+','+str(tw)+',3,t}$','$\Pi_{'+manifoldIn+','+str(tw)+',4,t}$', '$\Pi_{'+manifoldIn+','+str(tw)+',5,t}$']
    ...
    ########
    csExPostProjections25 = sl.cs(pd.read_sql('SELECT * FROM ' + manifoldIn + "_" + universe + '_RsExPostProjections_tw_25', conn).set_index('Dates', drop=True))
    fig, ax = plt.subplots()
...
def gDMAP_TES(mode, universe, alphaChoice):
    ...
    if mode == 'create':
        ...
        for x in tqdm(range(st, len(covMatList)+1)):
            ...
            for col in out.columns:
                ...
                resDF.to_sql('gDMAP_TES_' + universe + "_" + str(alphaChoice) + "_" + str(c), conn, if_exists='replace')
        ...
        #riskParityVol = np.sqrt(252) * sl.S(sl.rollStatistics(df, 'Vol', nIn=250)) * 100
        #df = (df / riskParityVol).replace([np.inf, -np.inf], 0)
        #df[df>1] = 0
        #shRP = np.sqrt(252) * sl.sharpe(sl.rs(df))
        #print(shRP)

        #dmapsCompAssetRetsList = []
        #dmapsCompRatesList = []
        #for pr in range(5):
        #    dmapsCompAssetsRets = pd.read_sql('SELECT * FROM DMAP_pyDmapsRun_AssetsRets_principalCompsDf_tw_100_'+str(pr), conn).set_index('Dates', drop=True)
        #    dmapsCompRates = pd.read_sql('SELECT * FROM DMAP_pyDmapsRun_Rates_principalCompsDf_tw_100_'+str(pr), conn).set_index('Dates', drop=True)

            #dmapsCompAssetRetsList.append(dmapsCompAssetsRets)
            #dmapsCompRatesList.append(dmapsCompRates)

            #dmapsCompAssetRetsList.append(sl.rs(dmapsCompAssetsRets))
            #dmapsCompRatesList.append(sl.rs(dmapsCompRates))

        #    dmapsCompAssetRetsList.append(sl.rs(dmapsCompAssetsRets).abs())
        #    dmapsCompRatesList.append(sl.rs(dmapsCompRates).abs())

        #dmapsCompAssetRetsDF = pd.concat(dmapsCompAssetRetsList, axis=1)
        #dmapsCompAssetRetsDF = sl.rowStoch(dmapsCompAssetRetsDF)
        #dmapsCompAssetRetsDF = sl.rowStoch(dmapsCompAssetRetsDF, mode='abs')
        #dmapsCompAssetRetsDF = sl.sign(dmapsCompAssetRetsDF)
        #dmapsCompAssetRetsDF = sl.cs(dmapsCompAssetRetsDF)

        #dmapsCompRatesDF = pd.concat(dmapsCompRatesList, axis=1)
        #dmapsCompRatesDF = sl.rowStoch(dmapsCompRatesDF)
        #dmapsCompRatesDF = sl.rowStoch(dmapsCompRatesDF, mode='abs')
        #dmapsCompRatesDF = sl.sign(dmapsCompRatesDF)
        #dmapsCompRatesDF = sl.cs(dmapsCompRatesDF)

        #sigDriver = sl.S(sl.fd(pd.read_sql('SELECT * FROM gDMAP_TES_AssetsRets_' + str(alphaChoice) + "_0", conn).set_index('Dates', drop=True))).fillna(0)
        sigDriver = sl.rs(sl.S(sl.fd(pd.read_sql('SELECT * FROM gDMAP_TES_Rates_' + str(alphaChoice) + "_0", conn).set_index('Dates', drop=True))).fillna(0))

        for pr in range(1,5):
            #sigDriver += sl.S(sl.fd(pd.read_sql('SELECT * FROM gDMAP_TES_AssetsRets_' + str(alphaChoice) + "_" + str(pr), conn).set_index('Dates', drop=True))).fillna(0)
            sigDriver += sl.rs(sl.S(sl.fd(pd.read_sql('SELECT * FROM gDMAP_TES_Rates_' + str(alphaChoice) + "_" + str(pr), conn).set_index('Dates', drop=True))).fillna(0))

        #sigDriver = sl.rs(sigDriver)
        sigDriver = sl.cs(sigDriver) # IT WORKS
        sigDriver = sl.sign(sigDriver)

        predictDF = df.mul(sigDriver, axis=0).fillna(0)
        sh_predictDF = np.sqrt(252) * sl.sharpe(predictDF)
        print(sh_predictDF.abs())
        rspredictDF = sl.rs(predictDF)
        sh_rspredictDF = np.sqrt(252) * sl.sharpe(rspredictDF)
        print(sh_rspredictDF)

        fig, ax = plt.subplots(nrows=3, ncols=1)
        mpl.pyplot.locator_params(axis='x', nbins=25)
        sigDriver.plot(ax=ax[0], legend=None, title='sigDriver')
        sl.cs(predictDF).plot(ax=ax[1], legend=None)
        sl.cs(rspredictDF).plot(ax=ax[2], legend=None)
        plt.show()
...
#RunRollManifold('AssetsRets')
#RunRollManifold('Rates')
...
#gDMAP_TES("run", "AssetsRets", 'sumKLMedian')
#gDMAP_TES("run", "Rates", 'sumKLMedian')
...
#ReturnsComparison('singleAsset')
#ReturnsComparison('portfolio')
