...
def TCA():
    for elem in [['PCA_250_19', 5, 'single'],['PCA_ExpWindow25_19', 2, 'single'],['PCA_100_4_Tail', 2, 'global_PCA'],
                 ['LLE_100_18', 200, 'single'],['LLE_ExpWindow25_0', 2, 'single'],['LLE_ExpWindow25_5_Head', 3, 'global_LLE'],
                 ['LLE_250_4_Head', 15, 'global_LLE']]:

        selection = elem[0]; Lag = elem[1];  co = elem[2]
        ...
        for scenario in ['Scenario1', 'Scenario2', 'Scenario3', 'Scenario4', 'Scenario5', 'Scenario6']:
            ...
            strat_name = selection+'_'+str(Lag)+'_'+str(co)+'_'+scenario
            strat_pnl_afterCosts.name = 'pnl'
            after_TCA_Sharpe = np.sqrt(252) * sl.sharpe(strat_pnl_afterCosts)
            print(scenario, ", ", after_TCA_Sharpe)

            strat_pnl_afterCosts.to_sql(strat_name+'_EMA_pnl_afterCosts', sqlite3.connect('TCA.db'), if_exists='replace')
